...
void VtBridge::setDefaultColors(const QColor &fg, const QColor &bg)
{
    ...
    VTermColor vfg, vbg;
    vterm_color_rgb(&vfg,
        static_cast<uint8_t>(fg.red()),
        static_cast<uint8_t>(fg.green()),
        static_cast<uint8_t>(fg.blue()));
    vterm_color_rgb(&vbg,
        static_cast<uint8_t>(bg.red()),
        static_cast<uint8_t>(bg.green()),
        static_cast<uint8_t>(bg.blue()));

    VTermState *state = vterm_obtain_state(m_vterm);
    vterm_state_set_default_colors(state, &vfg, &vbg);

    // Also update palette index 0 (black) and 7 (white) to match
    // so that programs using indexed colors get sensible defaults
    vterm_state_set_palette_color(state, 0, &vbg);
    vterm_state_set_palette_color(state, 7, &vfg);
    vterm_state_set_palette_color(state, 15, &vfg);

    qCDebug(logVt) << "Default colors updated — fg:" << fg.name()
                   << "bg:" << bg.name();
...
// ─── Color conversion ──────────────────────────────────────
//
// The critical insight: libvterm marks cells that use the
// terminal's default fg/bg with VTERM_COLOR_DEFAULT_FG/BG.
// These cells should use the HOST theme colors, not hardcoded
// palette values. We return invalid QColor for defaults, and
// the renderer substitutes ChiTheme colors.

QColor VtBridge::vtermColorToQColor(const VTermColor &c, bool *isDefault)
{
    if (isDefault) *isDefault = false;

    // Check for default colors FIRST — these must use theme colors
    if (VTERM_COLOR_IS_DEFAULT_FG(&c) || VTERM_COLOR_IS_DEFAULT_BG(&c)) {
        if (isDefault) *isDefault = true;
        return QColor();  // invalid → renderer uses theme color
    }
    ...
    if (VTERM_COLOR_IS_INDEXED(&c)) {
        ...
        // Indices 0 and 7/15 are the "default-like" colors.
        // We still return them as real colors here — the palette
        // was already updated via setDefaultColors() so they
        // should match the theme.

        // Standard 16 ANSI colors — Material 3 inspired
        ...
        if (idx < 232) {
            int ci = idx - 16;
            return QColor((ci / 36) * 51, ((ci / 6) % 6) * 51, (ci % 6) * 51);
        }
...
Cell VtBridge::vtermCellToCell(const VTermScreenCell &vc)
{
    ...
    // Color conversion — default colors come back as invalid QColor
    // so the renderer can substitute theme colors

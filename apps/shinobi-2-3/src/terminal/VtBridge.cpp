...
void VtBridge::setAnsiPalette(const QVector<QColor> &palette16)
{
    ...
    for (int i = 0; i < 16; ++i) {
        VTermColor vc;
        vterm_color_rgb(&vc,
            palette16[i].red(), palette16[i].green(), palette16[i].blue());
        vterm_state_set_palette_color(state, i, &vc);
    }

    qCDebug(logVt) << "ANSI 16-color palette updated from theme";
    emit contentChanged();
}
...
int VtBridge::onSetTermProp(VTermProp prop, VTermValue *val, void *user) {
    if (prop == VTERM_PROP_TITLE) {
        auto title = QString::fromUtf8(val->string.str, val->string.len);
        qCDebug(logVt) << "Title:" << title;
        emit static_cast<VtBridge*>(user)->titleChanged(title);
    }
    return 0;
}
...
QColor VtBridge::vtermColorToQColor(const VTermColor &c)
{
    // Default fg/bg → invalid QColor → renderer uses theme color
    ...
    if (VTERM_COLOR_IS_INDEXED(&c)) {
        // libvterm resolves indexed colors through the palette
        // we set via setAnsiPalette(), so we need to ask libvterm
        // to convert to RGB for us. Since we can't call the state
        // from a static context, we return as indexed and let the
        // cell use whatever libvterm resolved.
        //
        // But wait — vterm_screen_get_cell already resolves indexed
        // colors to RGB when the screen has palette set. So if we
        // reach here, it means libvterm couldn't resolve it.
        // Fall back to hardcoded palette.
        ...
        // indices 0-15: palette was set via setAnsiPalette,
        // libvterm should have resolved these to RGB already.
        // If we still get here, return invalid → theme default
        return QColor();
    }

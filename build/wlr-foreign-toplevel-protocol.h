 ...
 *
 * After a client binds the zwlr_foreign_toplevel_manager_v1, each
 * opened toplevel window will be sent via the toplevel event
 ...
 *
 * A zwlr_foreign_toplevel_handle_v1 object represents an opened
 * toplevel window. Each app may have multiple opened toplevels.
 *
...
struct zwlr_foreign_toplevel_handle_v1_listener {
	 ...
	 * zwlr_foreign_toplevel_handle_v1 is created and each time the
	 ...
	 * changes.
	 *
	 * No event is emitted when the toplevel is created; this event is
	 * only emitted afterwards when the parent of an already existing
	 * toplevel changes.
	 * @since 3
	 */
 ...
 * @ingroup iface_zwlr_foreign_toplevel_handle_v1
 *
 * The rectangle of the surface specified in this request corresponds
 * to the place where the app is placed in the taskbar or other
 * similar surfaces. This is used by the compositor for animations.
 */
static inline void
 ...
 *
 * This request should be called either when the client does not want
 * to use the toplevel anymore or after the closed event to finalize
 ...
 * @ingroup iface_zwlr_foreign_toplevel_handle_v1
 *
 * Requests that the toplevel be made fullscreen. If the fullscreen
 * state is to be set on a particular output this can be specified.
 */
static inline void
